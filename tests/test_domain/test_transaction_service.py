...
class TestTransactionService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_conduct_transaction_success(self):
        ...
        await self.transaction_service.conduct_transaction(self.from_user_id, "receiver_user", self.positive_amount)
        ...
        self.mock_balance_service.can_afford.assert_called_once_with(self.from_user_id, self.positive_amount)
        self.mock_transaction_repo.create_transaction.assert_called_once_with(self.from_user_id, self.to_user_id, self.positive_amount)
        self.mock_balance_service.conduct_transaction_fund_transfer.assert_called_once_with(self.from_user_id, self.to_user_id, self.positive_amount)
    ...
    async def test_conduct_transaction_negative_amount(self):
        with self.assertRaises(NegativeTransactionAmount):
            await self.transaction_service.conduct_transaction(self.from_user_id, "receiver_user", self.negative_amount)
    ...
    async def test_conduct_transaction_user_does_not_exist(self):
        ...
        with self.assertRaises(UserDoesntExists):
            await self.transaction_service.conduct_transaction(self.from_user_id, "non_existent_user", self.positive_amount)
    ...
    async def test_conduct_transaction_self_transfer(self):
        ...
        with self.assertRaises(IncorrectReceiverCredential):
            await self.transaction_service.conduct_transaction(self.from_user_id, "self", self.positive_amount)
    ...
    async def test_conduct_transaction_insufficient_funds(self):
        ...
        with self.assertRaises(InsufficientFunds):
            await self.transaction_service.conduct_transaction(self.from_user_id, "receiver_user", 50)
